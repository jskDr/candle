...
impl Model for Mlp {
    fn new(vs: VarBuilder) -> Result<Self> {
        ...
        let ln1 = candle_nn::linear(IMAGE_DIM, 20, vs.pp("ln1"))?;
        let ln2 = candle_nn::linear(20, LABELS, vs.pp("ln2"))?;
        Ok(Self { ln1, ln2 })
    }

    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        x.apply(&self.ln1)? // input x: &Tensor, output x: Tensor
         .relu()?
         .apply(&self.ln2)
    }
}
...
impl ConvNet {
    fn new(vs: VarBuilder) -> Result<Self> {
        let conv1 = candle_nn::conv2d(1, 8, 5, Default::default(), vs.pp("c1"))?; //28x28 -> 14x14
        let (conv2_o, conv2_f) = (16_usize, 4_usize);
        let conv2 = candle_nn::conv2d(8, conv2_o, 5, Default::default(), vs.pp("c2"))?; //14x14 -> 7x7
        let fc1_i = conv2_o * (7 - conv2_f + 1).pow(2);
        let fc1 = candle_nn::linear(fc1_i, 20, vs.pp("fc1"))?; // 32 * 7-5 * 7
        let fc2 = candle_nn::linear(20, LABELS, vs.pp("fc2"))?;
